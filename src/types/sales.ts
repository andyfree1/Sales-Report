...
export const getAdditionalCommission = (totalVolume: number, levels: CommissionLevel[] = DEFAULT_COMMISSION_LEVELS): number => {
...
export const calculateTotalCommission = (saleAmount: number, totalVolume: number, saleType: 'DEED' | 'TRUST', levels: CommissionLevel[] = DEFAULT_COMMISSION_LEVELS): number => {
  const baseCommission = getBaseCommission(saleAmount, saleType);
  const additionalCommission = getAdditionalCommission(totalVolume, levels);
